...
async function autoGenerateDailyChallenges(
  ...
  // Fetch context: servers + menu items + IA config
  const [servers, menuItems, restaurant] = await Promise.all([
    ...
    prisma.restaurant.findUnique({ where: { id: restaurantId }, select: { ollamaApiKey: true, ollamaLangModel: true } }),
  ]);
  ...
  // Try IA if PRO_IA and key exists
  if (subscription === "PRO_IA" && restaurant?.ollamaApiKey && restaurant.ollamaLangModel) {
    try {
...
Format : 3 lignes uniquement, sans numéro, sans tiret.`;
      ...
      if (provider === "openai" || provider === "mistral") {
        ...
        headers["Authorization"] = `Bearer ${restaurant.ollamaApiKey}`;
        ...
        headers["x-api-key"] = restaurant.ollamaApiKey;
        ...
        const res = await fetch(
          `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${restaurant.ollamaApiKey}`,
