...
async function autoGenerateDailyChallenges(
  ...
  // Try global IA config if nova_ia app is enabled
  const hasIa = await hasApp(restaurantId, "nova_ia");
  if (hasIa) {
    try {
...
export async function serverPortalRoutes(app: FastifyInstance) {
  ...
  app.post("/login", authRateLimit, async (req, reply) => {
    ...
    autoGenerateDailyChallenges(restaurant.id, server.id, restaurant.name).catch(() => {});

    const enabledApps = await getEnabledApps(restaurant.id);

    return {
      token,
      server: { id: server.id, name: server.name, photoUrl: server.photoUrl },
      restaurant: { id: restaurant.id, name: restaurant.name, enabledApps: [...enabledApps] },
    };
  });
  ...
  app.get("/me", async (req, reply) => {
    ...
    return { server: { ...server, globalChallenges }, restaurant: { ...restaurant, enabledApps: [...enabledApps] } };
  });
  ...
  app.post("/challenges/generate-daily", async (req, reply) => {
    ...
    const hasIa = await hasApp(me.restaurantId, "nova_ia");
    if (!hasIa) return reply.code(403).send({ error: "APP_NOT_ENABLED", app: "nova_ia" });
