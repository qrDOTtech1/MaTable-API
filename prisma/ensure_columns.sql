...
-- MenuItem: quantity discount tiers (legacy, % de remise)
ALTER TABLE "MenuItem" ADD COLUMN IF NOT EXISTS "quantityDiscounts" JSONB NOT NULL DEFAULT '[]'::jsonb;

-- MenuItem: quantity TIERS (nouveau format)
-- Format: [{ "qty": 3, "priceCents": 2200 }, { "qty": 6, "priceCents": 4000 }]
-- Le client choisit explicitement un palier dans la card du plat.
ALTER TABLE "MenuItem" ADD COLUMN IF NOT EXISTS "quantityTiers" JSONB DEFAULT '[]'::jsonb;
